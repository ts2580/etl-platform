...
public interface SalesforceOrgCredentialRepository {
    ...
    @Select("SELECT id, org_key AS orgKey, org_name AS orgName, my_domain AS myDomain, client_id AS clientId, client_secret AS clientSecret, access_token AS accessToken, refresh_token AS refreshToken, access_token_issued_at AS accessTokenIssuedAt, is_active AS isActive, is_default AS isDefault, created_at AS createdAt, updated_at AS updatedAt FROM config.salesforce_org_credentials WHERE org_key = #{orgKey} AND is_active = true")
