...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleAppException(AppException ex, HttpServletRequest request) {
        ...
        if (wantsHtml(request)) {
            return htmlError(HttpStatus.BAD_REQUEST, "BAD_REQUEST", "요청을 처리하지 못했어요", ex.getMessage(), "/login");
        }
                    ...
                    <main class=\"w-full max-w-2xl rounded-[28px] border border-rose-100 bg-white/95 p-6 shadow-xl shadow-indigo-100 sm:p-8\">
                        <div class=\"flex items-start gap-3\">
                            ...
                            <div class=\"min-w-0\">
                                ...
                                <p class=\"mt-3 text-sm leading-6 text-slate-500\">API 에러를 브라우저에서 바로 보기 좋게 바꾼 화면입니다. 자세한 원인은 서버 쉘 로그에도 함께 남겨뒀어요.</p>
                            </div>
                        </div>
