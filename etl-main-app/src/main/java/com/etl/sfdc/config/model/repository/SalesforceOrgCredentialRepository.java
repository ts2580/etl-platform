...
public interface SalesforceOrgCredentialRepository {
    ...
    @Insert("INSERT INTO config.salesforce_org_credentials (org_key, org_name, my_domain, schema_name, client_id, client_secret, access_token, refresh_token, access_token_issued_at, is_active, is_default) " +
            "VALUES (#{orgKey}, #{orgName}, #{myDomain}, #{schemaName}, #{clientId}, #{clientSecret}, #{accessToken}, #{refreshToken}, NOW(), COALESCE(#{isActive}, true), COALESCE(#{isDefault}, false)) " +
            ...
            "client_secret = VALUES(client_secret), access_token = VALUES(access_token), refresh_token = VALUES(refresh_token), " +
    ...
    @Select("SELECT id, org_key AS orgKey, org_name AS orgName, my_domain AS myDomain, schema_name AS schemaName, client_id AS clientId, client_secret AS clientSecret, access_token AS accessToken, refresh_token AS refreshToken, access_token_issued_at AS accessTokenIssuedAt, is_active AS isActive, is_default AS isDefault, created_at AS createdAt, updated_at AS updatedAt " +
    ...
    @Update("UPDATE config.salesforce_org_credentials SET is_active = #{isActive}, updated_at = CURRENT_TIMESTAMP WHERE org_key = #{orgKey}")
    int setActiveFlag(@Param("orgKey") String orgKey, @Param("isActive") boolean isActive);
