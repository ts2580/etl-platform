package com.etlplatform.common.storage.database.sql;

import com.etlplatform.common.storage.database.DatabaseVendor;

import java.util.List;

public class MySqlDatabaseVendorStrategy extends BaseDatabaseVendorStrategy {

    @Override
    public DatabaseVendor vendor() {
        return DatabaseVendor.MYSQL;
    }

    @Override
    public String quoteIdentifier(String identifier) {
        return "`" + identifier + "`";
    }

    @Override
    public String salesforceColumnType(String sfType, int length) {
        return switch (sfType) {
            case "id", "reference" -> "VARCHAR(18)";
            case "textarea" -> length > 4000 ? "TEXT" : "VARCHAR(" + length + ")";
            case "string", "picklist", "multipicklist", "phone", "url" -> "VARCHAR(" + Math.max(length, 1) + ")";
            case "boolean" -> "BOOLEAN";
            case "datetime" -> "DATETIME(6)";
            case "date" -> "DATE";
            case "time" -> "TIME(6)";
            case "double", "percent", "currency" -> "DOUBLE";
            case "int" -> "INT";
            default -> "VARCHAR(255)";
        };
    }

    @Override
    public String upsertValueReference(String columnName) {
        return "VALUES(" + quoteIdentifier(columnName) + ")";
    }

    @Override
    public String columnCommentClause(String comment) {
        return " COMMENT '" + normalizeComment(comment) + "'";
    }

    @Override
    public String buildUpsertSql(String qualifiedTableName,
                                 List<String> insertColumns,
                                 List<String> mutableColumns,
                                 String keyColumn,
                                 String freshnessColumn,
                                 String eventColumn) {
        StringBuilder sql = new StringBuilder();
        sql.append("INSERT INTO ").append(qualifiedTableName).append(" (");
        appendQuotedColumns(sql, insertColumns);
        sql.append(") VALUES (");
        appendPlaceholders(sql, insertColumns.size());
        sql.append(") ON DUPLICATE KEY UPDATE ");

        for (String column : mutableColumns) {
            sql.append(quoteIdentifier(column)).append(" = IF(")
                    .append(upsertValueReference(freshnessColumn)).append(" IS NULL OR ")
                    .append(quoteIdentifier(freshnessColumn)).append(" IS NULL OR ")
                    .append(upsertValueReference(freshnessColumn)).append(" >= ")
                    .append(quoteIdentifier(freshnessColumn)).append(", ")
                    .append(upsertValueReference(column)).append(", ")
                    .append(quoteIdentifier(column)).append("), ");
        }

        sql.append(quoteIdentifier(freshnessColumn)).append(" = GREATEST(COALESCE(")
                .append(quoteIdentifier(freshnessColumn)).append(", '1970-01-01 00:00:00'), COALESCE(")
                .append(upsertValueReference(freshnessColumn)).append(", '1970-01-01 00:00:00')), ")
                .append(quoteIdentifier(eventColumn)).append(" = GREATEST(COALESCE(")
                .append(quoteIdentifier(eventColumn)).append(", '1970-01-01 00:00:00'), COALESCE(")
                .append(upsertValueReference(eventColumn)).append(", '1970-01-01 00:00:00'))");
        return sql.toString();
    }

    private void appendQuotedColumns(StringBuilder sql, List<String> columns) {
        for (int i = 0; i < columns.size(); i++) {
            if (i > 0) {
                sql.append(", ");
            }
            sql.append(quoteIdentifier(columns.get(i)));
        }
    }

    private void appendPlaceholders(StringBuilder sql, int count) {
        for (int i = 0; i < count; i++) {
            if (i > 0) {
                sql.append(", ");
            }
            sql.append("?");
        }
    }
}
